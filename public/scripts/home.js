...
function onJson(json) {
    ...
    for (let i in json) {
        ...
        if(json[i].postnumber<=0){
            post.querySelector(".avatar img").src ="../public/imgs/1.jpg";
        }
        else if(json[i].postnumber>0 && json[i].postnumber<=4) {
            post.querySelector(".avatar img").src ="../public/imgs/2.jpg";
        }
        else if(json[i].postnumber>4 && json[i].postnumber<=9) {
            post.querySelector(".avatar img").src ="../public/imgs/3.jpg";
        }
        else {
            post.querySelector(".avatar img").src ="../public/imgs/4.jpg";
        }
        ...
        deletepic.src="../public/imgs/x.jpg";
...
function deletePost(event){
    ...
    fetch(BASE_URL+'/deletePost/' + deletedid).then(showPosts);
}
...
function showPosts(){
    fetch(BASE_URL+"/getPost").then(responsePost).then(onJson);
    showStats();
}
function showStats(){
...
function sendText(event){
    if(document.getElementById("text").value.length!=0){
        ...
        fetch(BASE_URL+'/addPost/'+text).then(closeModal);
    }
    closeModal();
...
function onJsonStats(json){
    ...
    fetch(BASE_URL+'/getSong/'+json[0].songReq).then(responseSong).then(onJsonSong);
}
function onJsonSong(json){
  ...
  for(let i=0;i<nRes;i++){
    ...
    play_button.src="../public/imgs/play.jpg";
